...
export const transformData = (
  ...
  transformedData.forEach((day) => {
    if (typeof day.count !== "number" || day.count < 0) {
      throw new Error(
        `Required property "count: number" missing or invalid. Got: ${day.count}`
      );
    }

    if (!/\d{4}-\d{2}-\d{2}/.test(day.date)) {
      throw new Error(
        `Required property "date: YYYY-MM-DD" missing or invalid. Got: ${day.date}`
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Required property "level: 0 | 1 | 2 | 3 | 4" missing or invalid. Got: ${day.level}.`
      );
    }
  });
...
export const fetchCalendarData = async (
  ...
  if (!response.ok) {
    throw Error(
      `Fetching LeetCode submission data for "${username}" failed: ${
